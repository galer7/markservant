...
describe('config.js', () => {
  ...
  beforeEach(async () => {
    ...
    testConfigDir = join(tmpdir(), `markservant-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('saveConfig', () => {
    ...
    it('pretty-prints JSON with 2-space indentation', async () => {
      const configToSave = {
        servers: [{ directory: '/test', port: 8080, pid: null, addedAt: '2024-01-15T10:00:00.000Z' }],
      };
    ...
    it('handles concurrent saves correctly', async () => {
      ...
      const saves = [
        saveConfig({ servers: [{ directory: '/path1', port: 3001, pid: null, addedAt: '2024-01-01T00:00:00.000Z' }] }),
        saveConfig({ servers: [{ directory: '/path2', port: 3002, pid: null, addedAt: '2024-01-02T00:00:00.000Z' }] }),
        saveConfig({ servers: [{ directory: '/path3', port: 3003, pid: null, addedAt: '2024-01-03T00:00:00.000Z' }] }),
      ];
  ...
  describe('findServer', () => {
    ...
    it('returns null when not found', async () => {
      ...
      await writeFile(
        configPath,
        JSON.stringify({
          servers: [{ directory: '/other/path', port: 3000, pid: null, addedAt: '2024-01-15T10:00:00.000Z' }],
        }),
        'utf-8'
      );
