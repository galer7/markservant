...
describe('tts-server.js', () => {
  ...
  beforeEach(async () => {
    ...
    testConfigDir = join(tmpdir(), `markservant-tts-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('startTtsServer', () => {
    ...
    it('starts container successfully and saves config', async () => {
      ...
      expect(docker.pullImageIfNeeded).toHaveBeenCalledWith('ghcr.io/remsky/kokoro-fastapi-cpu:latest');
  ...
  describe('stopTtsServer', () => {
    ...
    it('reports when container was not running', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        expect.stringContaining('was not running')
      );
    });
