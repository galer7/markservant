...
describe('add command', () => {
  ...
  beforeEach(async () => {
    ...
    testConfigDir = join(tmpdir(), `markservant-add-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    addServer.mockResolvedValue({ directory: '/test/dir', port: 9001, pid: null, addedAt: new Date().toISOString() });
  ...
  describe('path normalization', () => {
    ...
    it('handles path resolution errors', async () => {
      ...
      expect(consoleSpy.error).toHaveBeenCalledWith(
        expect.stringContaining('Cannot resolve path')
      );
      expect(consoleSpy.error).toHaveBeenCalledWith(
  ...
  describe('existing server handling', () => {
    it('shows existing server and opens Edge when already in watch list', async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('already being served')
      );
      expect(consoleSpy.log).toHaveBeenCalledWith(
  ...
  describe('LaunchAgent installation', () => {
    it('installs LaunchAgent on first server when not already installed', async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('Installed LaunchAgent')
      );
    });
  ...
  describe('success message', () => {
    it('shows success message with correct directory', async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('/my/markdown/folder')
      );
    });
    ...
    it('shows success message with correct URL', async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('http://localhost:9500')
      );
    });
