...
import type { MappedWord, SynthesizedChunk, TextChunk } from './tts/types.js';
...
/** Flat array of all mapped words across all chunks. */
let allMappedWords: MappedWord[] = [];
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const readAloudCmd = vscode.commands.registerCommand(
    'markservant-tts.readAloud',
    () => readAloud(context),
  );

  // Register the "Stop Reading" command
  const stopCmd = vscode.commands.registerCommand(
    'markservant-tts.stop',
    () => stopReading(),
  );
...
async function readAloud(context: vscode.ExtensionContext) {
  ...
  panel.onMessage('highlightWord', (msg: { index: number }) => {
    ...
    highlighter.highlightWord(
      activeEditor,
      mappedWord.sourceStart,
      mappedWord.sourceEnd,
    );
  });

  panel.onMessage('chunkEnded', (msg: { chunkIndex: number }) => {
    if (!isReading) return;
    playNextChunk(client, panel);
  });
  ...
  panel.onMessage('ready', () => {
    ...
    synthesizeAndPlay(client, panel, 0);
  });

  panel.onMessage('error', (msg: { message: string }) => {
    vscode.window.showErrorMessage(`TTS playback error: ${msg.message}`);
    stopReading();
  });
}
...
async function synthesizeAndPlay(
  ...
  try {
    ...
    if (chunkIndex + 1 < textChunks.length) {
      prefetchPromise = synthesizeChunk(
        client,
        textChunks[chunkIndex + 1],
        chunkIndex + 1,
      ).catch(() => null); // Don't fail the current playback if pre-fetch fails
    }
  } catch (err: unknown) {
