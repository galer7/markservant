...
const { installLaunchAgent, uninstallLaunchAgent, isLaunchAgentInstalled } = await import(
  "./launchagent.js"
);
...
describe("launchagent.js", () => {
  ...
  beforeEach(async () => {
    ...
    testLaunchAgentsDir = join(
      tmpdir(),
      `markservant-test-launchagent-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  describe("installLaunchAgent", () => {
    ...
    it("throws error with message when launchctl fails", () => {
      ...
      expect(() => installLaunchAgent()).toThrow(
        "Failed to load LaunchAgent: Service already loaded",
      );
    });
