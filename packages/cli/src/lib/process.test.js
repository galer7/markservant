...
import { isServerRunning, openInEdge, startServer, stopServer } from "./process.js";
...
describe("process.js", () => {
  ...
  describe("startServer", () => {
    it("spawns markserv with correct arguments using node and full path", () => {
      ...
      const expectedMarkservPath = `${require("node:path").dirname(process.execPath)}/markserv`;
      expect(spawn).toHaveBeenCalledWith(
        process.execPath,
        [
          expectedMarkservPath,
          "/path/to/directory",
          "-p",
          "8080",
          "--livereloadport",
  ...
  describe("openInEdge", () => {
    it("calls exec with correct command for opening URL in Edge", async () => {
      execCallback.mockImplementation((_cmd, callback) => {
