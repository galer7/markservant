...
import { isServerRunning, startServer } from "../lib/process.js";
...
export default async function startCommand() {
  ...
  for (const server of servers) {
    ...
    if (pid !== null && isServerRunning(pid)) {
      console.log(
        chalk.blue(`[${directory}]`) + chalk.gray(` Already running on port ${port} (PID: ${pid})`),
      );
      alreadyRunningCount++;
      continue;
    }
    ...
    try {
      ...
      console.log(chalk.blue(`[${directory}]`) + chalk.red(` Failed to start: ${error.message}`));
      failedCount++;
    }
  }
