...
vi.mock("../lib/process.js", () => ({
  startServer: vi.fn((_directory, _port) => 12345),
...
describe("start command", () => {
  ...
  beforeEach(async () => {
    ...
    testConfigDir = join(
      tmpdir(),
      `markservant-start-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
    ...
    startServer.mockImplementation((_directory, _port) => 12345);
  ...
  describe("no servers", () => {
    it('shows "No servers to start" when empty', async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("No servers to start"));
    });
  ...
  describe("starting stopped servers", () => {
    ...
    it("logs started servers with URL and PID", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("Started"));
      // Check for URL
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("http://localhost:9000"));
      // Check for PID
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("54321"));
    });
  });
  ...
  describe("already running servers", () => {
    ...
    it("logs already running servers", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("Already running"));
      // Check for port
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("9000"));
      // Check for PID
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("11111"));
    });
  ...
  describe("error handling", () => {
    it("handles startServer errors gracefully", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("Failed to start"));
  ...
  describe("summary counts", () => {
    it("shows correct summary when all servers started", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("2 started"));
    });
    ...
    it("shows correct summary when all servers already running", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("2 already running"));
    });
    ...
    it("shows correct summary when all servers fail", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("2 failed"));
    });
    ...
    it("shows correct summary with mixed results", async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("1 started"));
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("1 already running"));
      expect(consoleSpy.log).toHaveBeenCalledWith(expect.stringContaining("1 failed"));
    });
  });
  ...
  describe("multiple servers", () => {
    ...
    it("updates PIDs for all started servers", async () => {
      ...
      startServer.mockReturnValueOnce(11111).mockReturnValueOnce(22222);
  ...
  describe("dotfiles option", () => {
    ...
    it("handles mixed servers with and without dotfiles option", async () => {
      ...
      expect(startServer).toHaveBeenCalledWith("/server/without-dotfiles", 9002, {
        dotfiles: undefined,
      });
    });
  });
