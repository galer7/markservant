...
import { addServer, findServer, getAllServers, updateServerPid } from "../lib/config.js";
...
import { isServerRunning, openInEdge, startServer } from "../lib/process.js";
  ...
  try {
    ...
    if (existingServer) {
      ...
      if (existingServer.pid === null || !isServerRunning(existingServer.pid)) {
        ...
        try {
          const newPid = startServer(normalizedPath, existingServer.port, {
            dotfiles: existingServer.dotfiles,
          });
