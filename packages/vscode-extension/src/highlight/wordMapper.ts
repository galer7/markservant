...
function fuzzyMatchAt(text: string, pos: number, target: string): number {
  ...
  if (ti === normTarget.length) {
    ...
    while (pos + matchLen < text.length && /^[^\p{L}\p{N}\s]/u.test(text[pos + matchLen])) {
      matchLen++;
    }
    return matchLen;
  }
...
export function mapWordsToSource(
  ...
  for (const ts of timestamps) {
      ...
      if (strippedTts.length > 0) {
        for (let pos = cursor; pos < text.length; pos++) {
          ...
          if (strippedAhead.startsWith(strippedTts)) {
            ...
            let consumed = 0;
            let matched = 0;
            for (let i = pos; i < text.length && matched < strippedTts.length; i++) {
              consumed++;
              const ch = text[i];
    ...
    if (matchStart !== -1) {
      ...
      const mapped = plainOffsetToSource(matchStart, matchStart + matchLen, chunk.offsetMap);
      ...
      const guessMapped = plainOffsetToSource(cursor, cursor + ttsWord.length, chunk.offsetMap);
