...
import { plainOffsetToSource, stripMarkdown } from "../src/tts/markdownStripper.js";
...
function expectWordMapsBack(
  ...
  const { sourceStart, sourceEnd } = plainOffsetToSource(plainStart, plainEnd, offsetMap);
...
describe("plainOffsetToSource", () => {
  ...
  it("returns exact source positions for text after bold", () => {
    ...
    const { sourceStart, sourceEnd } = plainOffsetToSource(endStart, endEnd, offsetMap);
  ...
  it("returns plain text offsets as fallback for unmapped ranges", () => {
    ...
    const { sourceStart, sourceEnd } = plainOffsetToSource(paraOneEnd, paraOneEnd + 2, offsetMap);
  ...
  it("handles single character lookups", () => {
    ...
    const { sourceStart, sourceEnd } = plainOffsetToSource(bStart, bStart + 1, offsetMap);
  ...
  it("handles italic word source mapping precisely", () => {
    ...
    const { sourceStart, sourceEnd } = plainOffsetToSource(quickStart, quickEnd, offsetMap);
  ...
  it("handles link text source mapping", () => {
    ...
    const { sourceStart, sourceEnd } = plainOffsetToSource(docsStart, docsEnd, offsetMap);
  ...
  it("handles heading text source mapping", () => {
    ...
    const { sourceStart, sourceEnd } = plainOffsetToSource(titleStart, titleEnd, offsetMap);
...
describe("edge cases", () => {
  ...
  it("offset map entries are in order and non-overlapping", () => {
    ...
    for (let i = 0; i < offsetMap.length; i++) {
      ...
      if (i > 0) {
        expect(entry.plainStart).toBeGreaterThanOrEqual(offsetMap[i - 1].plainEnd);
      }
    }
  });
